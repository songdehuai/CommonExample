...
import com.songdehuai.widget.myrefreshlayout.footer.LoadingView;
import com.songdehuai.widget.myrefreshlayout.header.progresslayout.ProgressLayout;
import com.songdehuai.widget.myrefreshlayout.processor.AnimProcessor;
import com.songdehuai.widget.myrefreshlayout.processor.IDecorator;
import com.songdehuai.widget.myrefreshlayout.processor.OverScrollDecorator;
import com.songdehuai.widget.myrefreshlayout.processor.RefreshProcessor;
 ...
 *
 * @author songdehuai
 * @ClassName: com.songdehuai.myrefreshlayout.widget.refresh.MyRefreshLayout
 * @date 2017/8/14 11:14
 */

public class MyRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
    ...
    //是否允许进入越界回弹模式,默认不允许
    protected boolean enableOverScroll = false;
    ...
    public MyRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MyRefreshLayout, defStyleAttr, 0);
        try {
            mMaxHeadHeight = a.getDimensionPixelSize(R.styleable.MyRefreshLayout_rf_max_head_height, (int) DensityUtil.dp2px(context, 100));
            mHeadHeight = a.getDimensionPixelSize(R.styleable.MyRefreshLayout_rf_head_height, (int) DensityUtil.dp2px(context, 60));
            mMaxBottomHeight = a.getDimensionPixelSize(R.styleable.MyRefreshLayout_rf_max_bottom_height, (int) DensityUtil.dp2px(context, 100));
            mBottomHeight = a.getDimensionPixelSize(R.styleable.MyRefreshLayout_rf_bottom_height, (int) DensityUtil.dp2px(context, 60));
            mOverScrollHeight = a.getDimensionPixelSize(R.styleable.MyRefreshLayout_rf_overscroll_height, (int) mHeadHeight);
            enableRefresh = a.getBoolean(R.styleable.MyRefreshLayout_rf_enable_refresh, true);
            enableLoadmore = a.getBoolean(R.styleable.MyRefreshLayout_rf_enable_loadmore, true);
            isPureScrollModeOn = a.getBoolean(R.styleable.MyRefreshLayout_rf_pureScrollMode_on, false);
            isOverScrollTopShow = a.getBoolean(R.styleable.MyRefreshLayout_rf_overscroll_top_show, true);
            isOverScrollBottomShow = a.getBoolean(R.styleable.MyRefreshLayout_rf_overscroll_bottom_show, true);
            enableOverScroll = a.getBoolean(R.styleable.MyRefreshLayout_rf_enable_overscroll, false);
            floatRefresh = a.getBoolean(R.styleable.MyRefreshLayout_rf_floatRefresh, false);
            autoLoadMore = a.getBoolean(R.styleable.MyRefreshLayout_rf_autoLoadMore, false);
            enableKeepIView = a.getBoolean(R.styleable.MyRefreshLayout_rf_enable_keepIView, true);
            showRefreshingWhenOverScroll = a.getBoolean(R.styleable.MyRefreshLayout_rf_showRefreshingWhenOverScroll, true);
            showLoadingWhenOverScroll = a.getBoolean(R.styleable.MyRefreshLayout_rf_showLoadingWhenOverScroll, true);
        } finally {
            a.recycle();
        }
    ...
    public class CoContext {
        ...
        public void onPullingDown(float offsetY) {
            pullListener.onPullingDown(MyRefreshLayout.this, offsetY / mHeadHeight);
        }

        public void onPullingUp(float offsetY) {
            pullListener.onPullingUp(MyRefreshLayout.this, offsetY / mBottomHeight);
        }

        public void onRefresh() {
            pullListener.onRefresh(MyRefreshLayout.this);
        }

        public void onLoadMore() {
            pullListener.onLoadMore(MyRefreshLayout.this);
        }
        ...
        public void onPullDownReleasing(float offsetY) {
            pullListener.onPullDownReleasing(MyRefreshLayout.this, offsetY / mHeadHeight);
        }

        public void onPullUpReleasing(float offsetY) {
            pullListener.onPullUpReleasing(MyRefreshLayout.this, offsetY / mBottomHeight);
        }

        public boolean dispatchTouchEventSuper(MotionEvent ev) {
            return MyRefreshLayout.super.dispatchTouchEvent(ev);
        }
