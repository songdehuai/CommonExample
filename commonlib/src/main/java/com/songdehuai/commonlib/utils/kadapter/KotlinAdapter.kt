...
abstract class KotlinAdapter<T> : RecyclerView.Adapter<KotlinAdapter.ViewHolder>() {
    ...
    fun bindData(
        type: Int,
        interceptBind: (type: Int, holder: ViewHolder, data: T?, backupData: Any?) -> Unit
    ) {
    ...
    private lateinit var mBind: ((type: Int, holder: ViewHolder, data: T) -> Unit?)

    private var mBindInterceptView: ((type: Int, holder: ViewHolder, data: T?, backupData: Any?) -> Unit?)? =
        null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (getItemViewType(position) == HEAD_TYPE) {
            mBindHeader(holder.itemView)
        } else if (getItemViewType(position) == FOOT_TYPE) {
            mBindFooter(holder.itemView)
        } else {
            ...
            if (isIntercept(getItemViewType(position))) {
                mBindInterceptView =
                    interceptViews.get(getItemViewType(position)) as ((type: Int, holder: ViewHolder, data: T?, backupData: Any?) -> Unit?)?
                ...
                mDatas.get(calculatePosition).data?.let {
                    mBind(
                        getItemViewType(position),
                        holder,
                        it
