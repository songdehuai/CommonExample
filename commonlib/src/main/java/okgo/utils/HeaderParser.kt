...
object HeaderParser {
    ...
    fun <T> createCacheEntity(
        ...
        if (cacheMode == CacheMode.DEFAULT) {
            ...
            if (!TextUtils.isEmpty(cacheControl)) {
                ...
                while (tokens.hasMoreTokens()) {
                    val token =
                        tokens.nextToken().trim { it <= ' ' }.toLowerCase(Locale.getDefault())
                    if (token == "no-cache" || token == "no-store") {
                        ...
                        try {
                            //获取最大缓存时间
                            maxAge = java.lang.Long.parseLong(token.substring(8))
                            //服务器缓存设置立马过期，不缓存
        ...
        for (headerName in responseHeaders.names()) {
            headers.put(headerName, responseHeaders[headerName])
        }

        //构建缓存实体对象
        val cacheEntity = CacheEntity<T>()
        cacheEntity.key = cacheKey
        cacheEntity.data = data
        cacheEntity.localExpire = localExpire
        cacheEntity.responseHeaders = headers
        return cacheEntity
    }
     ...
     */
    fun <T> addCacheHeaders(
        request: Request<*, *>,
        cacheEntity: CacheEntity<T>?,
        cacheMode: CacheMode
    ) {
        //1. 按照标准的 http 协议，添加304相关请求头
        if (cacheEntity != null && cacheMode == CacheMode.DEFAULT) {
            val responseHeaders = cacheEntity.responseHeaders
            responseHeaders?.run {
                val eTag = get(HttpHeaders.HEAD_KEY_E_TAG)
                if (eTag != null) request.headers(HttpHeaders.HEAD_KEY_IF_NONE_MATCH, eTag)
                val lastModified =
                    HttpHeaders.getLastModified(get(HttpHeaders.HEAD_KEY_LAST_MODIFIED))
                if (lastModified > 0) request.headers(
